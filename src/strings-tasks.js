...
function removeFirstOccurrences(str, value) {
  ...
  if (str.includes(value)) {
    ...
    const val1 = str.slice(0, idx, idx + value.length);
    const val2 = str.slice(idx + value.length);
...
function sumOfCodes(str) {
  // return [...str].reduce(
  //   (accumulator, currentValue) => accumulator + currentValue.charAt(0)
  // );
...
function encodeToRot13(str) {
  ...
  arr.forEach((el) => {
    ...
    val = val.map((e) => alphabet[alphabet.indexOf(e) + 13]).join('');
    result.push(val.concat(char));
  });
